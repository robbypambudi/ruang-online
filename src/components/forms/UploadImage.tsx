...
export default function DropzoneStoreInput({
  ...
  >(
    useMutation(async (data) => {
      await axios
        .post<ApiResponse<{ url: string }>>('/api/upload', data, {
          headers: {
  ...
  const { mutate: hanldeDeleteFile, isLoading: onDeleteLoading } =
    useMutationToast<void, string>(
      useMutation(async (data) => {
        await axios
          .delete<ApiResponse<void>>(`/api/upload?key=${data}`)
          .then((res) => {
            return res;
          });
      })
    ...
    <div className='w-full space-y-1.5 rounded-md'>
      {label && (
        <label htmlFor={id} className='flex space-x-1'>
          <Typography
            variant='c1'
            className='text-typo-primary text-sm font-medium'
          >
            {label}
          </Typography>
          {validation?.required && (
        ...
        <Controller
          ...
          render={() => (
            ...
            >
              ...
              <div
                className={clsxm(
                  'bg-base-white w-full cursor-pointer rounded-md',
                  'flex flex-col items-center space-y-2 px-3 py-8',
                  'border-typo-inline border-2 border-dashed ',
                  'relative',
                  error
                    ? 'border-danger-40 group-focus:border-danger-50'
                    : 'group-hover:border-typo-primary group-focus:border-typo-primary',
                  className
                )}
              >
                {isLoading ? (
                  <div
                    role='status'
                    className='bg-gra absolute left-1/2 top-2/4 -translate-x-1/2 -translate-y-1/2'
                  >
                    <svg
                      aria-hidden='true'
                  ...
                  <>
                    <div className='text-typo-label h-8 w-8'>
                      <MdOutlineAddPhotoAlternate className='h-full w-full' />
                    </div>

                    <div className='flex flex-col items-center'>
                      <Typography
                        variant='c1'
                        className='text-typo-icon text-center text-sm'
                      >
                        <span className='text-typo-primary font-semibold underline'>
                          Klik untuk upload
                        </span>{' '}
                        atau drag and drop
                      </Typography>
                    ...
                    <Typography
                      variant='c2'
                      className='text-typo-primary text-center text-xs'
                    >
                      Tersisa 1 file lagi
                    </Typography>
                  </>
      ...
      {!hideError && error && <ErrorMessage id={id} />}
