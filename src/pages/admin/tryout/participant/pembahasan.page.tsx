...
export type DetailPembahasanSoalAdmin = {
  id: string;
  question: string;
  type: string;
  category: string;
...
export default withAuth(DetailPembahasanSoalAdmin, [
  'admin_tryout_participant.index',
]);
function DetailPembahasanSoalAdmin() {
  const router = useRouter();
  const methods = useForm<SearchForm>();

  const { quiz_list_id, username, user_id } = router.query;

  const { watch } = methods;

  const url = `/admin/quiz_list/user/detail?quiz_list_id=${quiz_list_id}&user_id=${user_id}`;

  const { data: queryData } = useQuery<
    ApiResponse<DetailPembahasanSoalAdmin[]>
  >([url], {
    ...
    <DashboardLayout>
      ...
      <header className='flex justify-between'>
        <div className='flex items-center gap-3'>
          <IconLink
            href={`/dashboard/tryout/result/${quiz_list_id}`}
            icon={FiArrowLeft}
      ...
      <main className='mt-6'>
        ...
        <section className='mt-6'>
          <div>
            {listSoal.map((item: DetailPembahasanSoalAdmin, index: number) => {
              const { question, type, name, answers, category } = item;

              const isAllCorrect = answers.some(
                (item) => item.is_correct > 0 && item.is_user_answered === true
              );
                ...
                >
                  <div className='rounded-xl border border-outline p-4'>
                    <div className='flex w-full justify-between'>
                      <div>
                        <Tag
                          color={isAllCorrect ? 'success' : 'danger'}
                          size='sm'
                        >
                          {isAllCorrect ? 'Benar' : 'Salah'}
                        </Tag>
